...
define(function (require) {
    ...
    var InfiniteScroll = function(opt) {
        ...
        var me = this;
        // 新加载效果
        var loadingAniHtml = '<div class="b-infinitescroll-next c-line-bottom">'
            + '<span class="b-infinitescroll-icon"></span><span class="b-infinitescroll-text">{{text}}</span></div>';
        var loadStatusText = ['正在加载...', '加载失败 点击重新加载', '到底了 没有更多内容了'];
        ...
        me.options = $.extend({
            ...
            scrollPageClass: 'b-infinitescroll-page',   // 内容列表每页className
            loadingHtml: loadingAniHtml.replace('{{text}}', loadStatusText[0]),                    // 正在加载文案效果
            loadFailHtml: loadingAniHtml.replace('{{text}}', loadStatusText[1]),                   // 加载失败文案效果
            loadOverHtml: loadingAniHtml.replace('{{text}}', loadStatusText[2]),                   // 加载完成文案效果
            bufferHeightPx: 10,
            pageResultNum: 10,
            limitShowPn: 2,
            preLoadPn: 1
    ...
    InfiniteScroll.prototype = {
        ...
        _init: function() {
            ...
            if (me.options.firstResult.length) {
                ...
                me.options.onPageRendered && me.options.onPageRendered.call(me, me.options.$result.children().get(0));
            };
        ...
        _bindScroll: function() {
            ...
            me.options.$wrapper.on('scroll' + me.eventSpace, function(e, withoutFirstResult) {
                ...
                // 到底了
                // 发现ios safari 手百上对于window.innerHeight的计算有问题，会导致我们此处的算法为false，无法触发渲染
                // 所以增加withoutFirstResult来规避问题
                if ((me.currentScrollTop >= me.scrollerHeight - me.wrapperHeight - me.options.bufferHeightPx) || withoutFirstResult) {
                    me._scrollBottomFn(withoutFirstResult);
                    // 执行回调
         ...
         * 当滚动条滚动到页面底部时执行
         */
        _scrollBottomFn: function(withoutFirstResult) {
            var me = this;
                ...
                $.when(dataDeferred).then(
                    ...
                    function (newResultArr) {
                        ...
                        if (newResultArr == 'NULL') {
                            ...
                            me.options.$wrapper.trigger('scroll', withoutFirstResult);
                        };
                    // 失败
                    }, function () {
                        // 标记数据状态为请求失败
        ...
        _updateScrollElement: function(pn) {
            ...
            me.options.onPageRendered && me.options.onPageRendered.call(me, $dom_newPage && $dom_newPage.get(0));
            ...
            if ($dom_shouldShowElement.length) {
                ...
                $dom_shouldShowElement.each(function(){
                    ...
                    if ($this.hasClass(recycleClass)) {
                        ...
                        me.options.onPageRendered && me.options.onPageRendered.call(me, $this.get(0));
                    };
                });
    ...
    var BInfiniteScroll = register('b-infinitescroll', {
        props: {
            loadingHtml: {
                type: String,
                value: '加载中'
        ...
        css: ".b-infinitescroll-next {  height: 50px;  line-height: 50px;  text-align: center;  background-color: #fff;}.b-infinitescroll-icon {  width: 12px;  height: 12px;  line-height: 12px;  border-radius: 50%;  display: inline-block;  vertical-align: middle;  background-color: #2a2a31;  opacity: .3;  -webkit-animation: b-infinitescroll-iconAni 750ms;  animation: b-infinitescroll-iconAni 750ms;  -webkit-animation-timing-function: linear;  animation-timing-function: linear;  -webkit-animation-iteration-count: infinite;  animation-iteration-count: infinite;}.b-infinitescroll-text {  font-size: 12px;  margin-left: 13px;  vertical-align: middle;  line-height: 12px;}@-webkit-keyframes b-infinitescroll-iconAni {  from {    opacity: .3;    -webkit-transform: rotateY(0deg);    transform: rotateY(0deg);  }  40% {    -webkit-transform: rotateY(90deg);    transform: rotateY(90deg);  }  50% {    opacity: .6;    -webkit-transform: rotateY(180deg);    transform: rotateY(180deg);  }  60% {    -webkit-transform: rotateY(270deg);    transform: rotateY(270deg);  }  to {    opacity: .3;    -webkit-transform: rotateY(360deg);    transform: rotateY(360deg);  }}@keyframes b-infinitescroll-iconAni {  from {    opacity: .3;    -webkit-transform: rotateY(0deg);    transform: rotateY(0deg);  }  40% {    -webkit-transform: rotateY(90deg);    transform: rotateY(90deg);  }  50% {    opacity: .6;    -webkit-transform: rotateY(180deg);    transform: rotateY(180deg);  }  60% {    -webkit-transform: rotateY(270deg);    transform: rotateY(270deg);  }  to {    opacity: .3;    -webkit-transform: rotateY(360deg);    transform: rotateY(360deg);  }}",
        holdChild: true,
        // 视窗
        $wrapper: $(window),
        // 滚动容器
        ...
        start: function () {
            ...
            if (!this.infiniteScroll) {
                this.infiniteScroll = new InfiniteScroll({
                    $result: this.$('.b-infinitescroll-result'),
                    $wrapper: this.$wrapper,
                    $scroller: this.$scroller,
                    $loading: this.$('.b-infinitescroll-loading'),
                    bufferHeightPx: this.getProp('bufferHeightPx'),
                    pageResultNum: this.getProp('pageResultNum'),
                    limitShowPn: this.getProp('limitShowPn'),
                    preLoadPn: this.getProp('preLoadPn'),
                    firstResult: this.firstResult,
                    pushResult: this.pushResult,
                    onScrollTop: function () {
                        _this.trigger('scrollTop');
                    },
                    onScrollBottom: function () {
                        _this.trigger('scrollBottom');
                    },
                    onChangeShowPN: function (np, lp) {
                        _this.trigger('changeShowPN', {
                            np: np,
                            lp: lp
                        });
                    },
                    onLoadNewPage: function (pn) {
                        _this.trigger('loadNewPage', pn);
                    },
                    onPageRendered: function (dom) {
                        _this.trigger('pageRendered', dom);
                    }
                });
            }

            var infiniteScroll = this.infiniteScroll;
        ...
        detached: function () {
            /**
             * sf 缓存策略会移动dom，导致组件的 detached 被触发
             * 所以此处判断组件 state，如果是 pause，表示用户后续可能需要恢复
             * 那么就暂时不销毁组件
             * 反之，则销毁组件
             */
            var infiniteScroll = this.infiniteScroll;
            if (!!infiniteScroll) {
                if (infiniteScroll.state == 'pause') {
