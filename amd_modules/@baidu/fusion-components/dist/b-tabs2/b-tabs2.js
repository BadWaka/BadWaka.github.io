/* eslint-disable fecs-valid-jsdoc, fecs-camelcase, no-use-before-define, radix, fecs-properties-quote, brace-style, no-unused-vars, fecs-valid-class-jsdoc, one-var */
...
define(function (require) {
    ...
    var _init = function (opt) {
        ...
        this.viewcontent = getClosestChild($panel, '.' + _this.contViewClass); // .find('.' + _this.contViewClass);       // 内容可视区dom
        this.contBox = getClosestChild($panel, '.' + _this.contBoxClass); // .find('.' + _this.contBoxClass);       // 内容实际区域dom

        // 算出tab内容总宽度和每个的宽度
        this.total_tab_width = 100 * this.navs.length;
        this.every_tab_width = 100 / this.navs.length;
        // 获取初始所在tab高度
        this.initial_height = null;
        // 容器总宽度和每个宽度
        this.contBox.width(this.total_tab_width + '%');
        this.conts.each(function (index, val) {
            $(val).css({
                'display': 'block',
                'width': _this.every_tab_width + '%',
                'height': 'auto'
            });
        });
        // 初始化
        this.navs.each(function (index, val) {
            if ($(val).hasClass('c-tabs-nav-selected')) {
                _this.contBox.css({
                    '-webkit-transform': 'translate3d(' + '-' + index * _this.every_tab_width + '%' + ', 0, 0)',
                    'transform': 'translate3d(' + '-' + index * _this.every_tab_width + '%' + ', 0, 0)',
                ...
                _this.conts[index].style.height = 'auto';
                _this.initial_height = _this.conts[index].offsetHeight;
            }
        });
        // 恢复动画执行时间
        this.contBox.css({
        ...
        // 设置可视层高度，否则初次加载时无具体高度导致第一次高度变换动画不实现
        this.viewcontent.height(this.initial_height + 'px');
        // 防止初始时内容为空的元素高度为0而被后面的内容挤掉
        this.conts.each(function (index, val) {
            if (!$(val).text()) {
                $(val).html('<div style="position: relative; top: -1px; height: 1px"></div>');
            }
        });
            ...
            require(['../deps/iscroll'], function (IScroll) {
                ...
                if (_this.current > 0 && _this.current < _this.sum) {
                    ...
                    if ($nav.length && ($nav.position().left + $nav.width()) > _this.view.width()) {
                        _this.tabScroll.scrollToElement(_this.navs[_this.current], 500,
                             _this.scrollSize, 0, '', {autoScroll: true});
                    }
                }
        ...
        _setEvents = function () {
            ...
            $.each(_this.navs, function (i, v) {
                ...
                $v.on('click', function () {
                    ...
                    _this.changeContent(_this.last, _this.current);
    ...
    var Tabs = function (panel, options) {
        ...
        this.contViewClass = options.contViewClass || 'c-tabs-view-content';
        this.contBoxClass = options.contBoxClass || 'c-tabs-tabcontent';
    ...
    $.extend(Tabs.prototype, {
        ...
        //     }
        // },
        changeContent: function (pre, nex) {
            // 获取点击tab的高度
            this.nowclicktab_height = this.conts.eq(nex).height();
            // 获取前后高度差值
            this.d_value = this.nowclicktab_height - this.initial_height;
            // 点击高度与原高度相等则不执行动画
            if (this.nowclicktab_height === this.initial_height) {
                this.viewcontent[0].className += ' c-tabs-stopanimate';
            }
            // 执行动画
            else {
                this.viewcontent[0].style.height = this.initial_height + this.d_value + 'px';
            }
            // 因等高时清空了动画所以在这里恢复
            this.viewcontent[0].className = 'c-tabs-view-content c-tabs-view-content-anim';
            // 计算移动百分比并移动
            this.move_distance = -(nex * this.every_tab_width) + '%';
            this.contBox.css({
                '-webkit-transform': 'translate3d(' + this.move_distance + ',0,0)',
                'transform': 'translate3d(' + this.move_distance + ',0,0)'
            });
            // 更新上一个tab的高度及tab内容容器高度
            this.initial_height = this.conts[nex].offsetHeight;
        },
