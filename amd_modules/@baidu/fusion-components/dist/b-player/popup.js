...
define(function (require) {
    ...
    PopupFrame.prototype = {
        ...
        */
        ...
        /*
         * 创建.b-player-popup-wrapper父容器单例,所有pop内容都append到这个dom中
         */
        _preparePopupWrapper: function () {
            var me = this;
            var popWrapperDom = $('.b-player-popup-wrapper');
            if (popWrapperDom.length) {
                me.$popupFrame = popWrapperDom;
                me.$popupFrame.empty();
            } else {
                me.$popupFrame = $('<div class="b-player-popup-wrapper"></div>');
        ...
        */
        _bindEvent: function () {
            var me = this;
            // mask遮罩和绑定退场事件
            me.$popupFrame.on('click', '.b-player-popup-mask,.b-player-popup-remove,.b-player-popup-modal', function () {
                me.closePopup();
            });
        },
        /*
        ...
        */
        _randerContent: function () {
            var me = this;
            // 遮罩层
            me.$popupMask = $('<div class="b-player-popup-mask"></div>');
            // modal层
            me.$popupModal = $('<div class="b-player-popup-modal"></div>');
            // modal内content
            me.$popupContent = $('<div class="b-player-popup-content"></div>');
            // modal内head
            me.$popupHead = $('<div class="b-player-popup-head"></div>');
            // 装填head内容
            if (me.options.title) {
                var titleWrapper = $('<div class="b-player-popup-title"></div>');
                titleWrapper.append(me.options.title);
                me.$popupHead.append(titleWrapper);
            }
            var remove = $('<div class="b-player-popup-remove c-icon">&#xe61a</div>');
        ...
        */
        _destroy: function () {
            var me = this;
            // 解绑事件
            me.$popupFrame.off('click', '.b-player-popup-mask,.b-player-popup-remove,.b-player-popup-modal');
            // 清除dom
            me.$popupFrame.empty();
        }
    };
